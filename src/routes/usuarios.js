...
router.post('/usuario/registro', async (req, res)=>{
    const { numeroCedula,
           ...
           const errors=[];
            ...
            if (!numeroCedula) {
             ...
             if (numeroCedula.length < 5) {
                errors.push({text: 'La cedula no es válida, debe tener al menos 5 digitos'});
            }
             if (!nombre) {
                errors.push({text: 'El nombre no es valido'});
            }
             if (!apellido) {
                errors.push({text: 'El apellido no es valido'});
            }
             if (!email) {
           ...
           errors.push({text: 'el email no es correcto'})
            ...
            if (errors.length > 0) {
            	res.render('usuario/registro_usuario',{ errors,
            	 numeroCedula,
				fechaExpedicion,
				ciudadExpedicion,
	            nombre,
...
router.post('/usuario/step',[
    check('numeroCedula').not().withMessage('').isNumeric().isLength({ min: 5, max:5 }).withMessage('Name must have more than 5 characters'),
